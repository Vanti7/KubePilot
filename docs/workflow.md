...
- [x] **Tests du reste du `store`** (2026-08-21) — dernier morceau du checklist beta côté tests. 9 nouveaux fichiers (`clusters`, `helm`, `helm_repos`, `registries`, `namespaces`, `secrets`, `metrics`, `integrations`, `exception_rules`), nouveau helper partagé `store_test_helpers_test.go` (migre tout via `models.AllModels()`). **3 bugs réels trouvés et corrigés** :
  ...
  - Seul item restant du checklist beta : Playwright (frontend)
...
- **Versioning** : SemVer + cycle alpha/beta/rc/stable (cf. CLAUDE.md). Version courante : `v0.2.0-alpha.12`.
