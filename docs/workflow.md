...
- [x] **Tests d'intégration frontend (Playwright)** (2026-08-23) — **dernier item du checklist beta, checklist complète.** `frontend/e2e/`, config à deux `webServer` (backend démo sur :8090, frontend Vite sur :3001 — ports dédiés pour ne jamais collisionner avec une instance de dev déjà lancée), backend démarré sur une base SQLite fraîche à chaque run (répertoire temp unique, pas de nettoyage nécessaire — un `rm` shell classique n'est pas portable sur `cmd.exe`). 21 tests : auth (redirection non connecté, mauvais mdp, connexion, déconnexion), fumée de navigation sur les 14 routes protégées, cycle créer/pause/supprimer d'une exception rule via l'UI réelle. `tsconfig.json` dédié dans `e2e/` (le tsconfig racine ne couvre que `src/`, sans ça les tests échappaient silencieusement à `tsc`). `npm run test` (déjà appelé par `make test-frontend`, jusqu'ici sans script défini)
  - **A trouvé un vrai bug produit** : l'intercepteur Axios redirigeait (`window.location.href`, rechargement complet) sur **tout** 401, y compris celui de `/auth/login` en cas de mauvais mot de passe — effaçait le message d'erreur avant que `Login.tsx` ait pu l'afficher. Corrigé : `/auth/login` exclu de la redirection auto
- [x] **Tests du reste du `store`** (2026-08-21) — dernier morceau du checklist beta côté tests backend. 9 nouveaux fichiers (`clusters`, `helm`, `helm_repos`, `registries`, `namespaces`, `secrets`, `metrics`, `integrations`, `exception_rules`), nouveau helper partagé `store_test_helpers_test.go` (migre tout via `models.AllModels()`). **3 bugs réels trouvés et corrigés** :
  - `UpsertNode`/`UpsertSecret` avaient le même défaut qu'`UpsertWorkload` (2026-08-01) : ID fantôme après `ON CONFLICT DO UPDATE`, jamais détecté faute de test. Même correctif (relecture de l'ID existant avant l'insertion). Sans conséquence connue aujourd'hui (rien n'utilise l'ID juste après l'appel), mais latent pour tout futur code qui s'y fierait
  - `ExceptionRule` créée avec `is_active:false` redevenait active malgré la demande — piège GORM sur les bools à tag `default:` (voir décision détaillée ci-dessous). `store.CreateExceptionRule` gagne un paramètre `isActive bool` explicite ; câblage handler mis à jour, validé en direct
- [x] **Page History** (2026-08-21) — l'API `GET /api/v1/action-logs` existait déjà (fondations audit trail), seule la page manquait (`disabled`/`V2` dans la sidebar depuis le début). Table paginée + filtres action/entité/statut + slide-over détail (`details` JSONB, IP, user-agent). Zéro changement backend. Dernier item du checklist beta côté fonctionnalités
...
### En continu
- [x] Tests backend — **complet au 2026-08-21**. `internal/watcher/tags_test.go` (semver), `internal/k8sops` (scale/restart/deploy/set-image), `internal/netguard` (SSRF), `internal/scoring/engine_test.go` (formule complète + CVSS + fenêtres de maintenance + exception rules, Example 1 de la doc reproduit exactement), et l'intégralité d'`internal/store` (findings, workloads, clusters, helm, helm_repos, registries, namespaces, secrets, métriques, integrations, exception_rules) — 3 bugs réels trouvés au passage (voir « Fait récemment » et `CHANGELOG.md`)
- [x] Tests frontend (Playwright) — **complet au 2026-08-23**. `frontend/e2e/`, 21 tests (auth, navigation, cycle CRUD exception rules) — 1 bug réel trouvé au passage (intercepteur Axios, voir « Fait récemment »)
...
- **Versioning** : SemVer + cycle alpha/beta/rc/stable (cf. CLAUDE.md). Version courante : `v0.2.0-alpha.13`.
