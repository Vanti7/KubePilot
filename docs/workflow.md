...
- **`docs/scoring.md` décrit un design plus ambitieux que `internal/scoring/engine.go` réel** — trouvé en écrivant `engine_test.go` (2026-08-06), pas encore réconcilié :
  - Le facteur CVSS (poids 20 %) est **câblé à 0 en dur** (« Reserved for future scanner integration ») — aucun des 3 exemples chiffrés de la doc (§8) n'est reproductible tel quel avec le code actuel (l'exemple 1 suppose un CVSS 7.8 pris en compte).
  - `isWindowActive` **retourne toujours `false`** (« Placeholder — safe default »)  — une `MaintenanceWindow` active n'a aujourd'hui aucun effet sur le score, contrairement à l'exemple 3 de la doc. Épinglé explicitement par `TestScoreFinding_MaintenanceWindowPlaceholder` : si `isWindowActive` est un jour implémenté pour de vrai, ce test échouera — c'est voulu, pour forcer une mise à jour consciente plutôt qu'un score qui change silencieusement.
  - Clé d'annotation différente : le code lit `kubepilot/criticality`, `kubepilot/rollback`, `kubepilot/exposure` (sans préfixe de domaine) ; la doc documente `kubepilot.io/criticality` (avec `.io`) et ne mentionne pas d'annotation pour rollback/exposure du tout (elle les décrit comme déduits par le collector, pas déclarés).
  - Pas de `ExceptionRule` évalué dans `ScoreFinding` malgré la section 9 de la doc, pas de signature `Compute(ctx, FindingContext)` pure malgré la section 12 (le vrai `ScoreFinding` prend `(finding, workload, cluster)` et écrit en base directement).
  - **Pas corrigé ni tranché** : `engine_test.go` teste le comportement réel du code (source de vérité retenue pour les tests), pas celui décrit dans la doc. Décision à prendre par l'utilisateur : mettre `docs/scoring.md` à jour pour refléter l'implémentation MVP, ou implémenter les briques manquantes (CVSS, fenêtres de maintenance, exceptions) pour rejoindre la doc.
...
- **Versioning** : SemVer + cycle alpha/beta/rc/stable (cf. CLAUDE.md). Version courante : `v0.2.0-alpha.8`.
