...
- **Chart Helm `helm/kubepilot/`** — listé comme livré mais absent du dépôt ; tout le tier in-cluster en dépendait. Deployment backend + frontend, Services, ServiceAccount, ClusterRole/Binding, Secret (JWT généré et **conservé** entre `helm upgrade`), PVC, Ingress, token d'intégration, NOTES
  - **RBAC** : `get nodes/proxy` (+ `nodes/stats`, `nodes/metrics`) pour les métriques agentless via kubelet, et lecture des `secrets` (nécessaire au décodage des releases Helm)
  - **Sans dépendance externe par défaut** : SQLite sur PVC + cache mémoire (`storage.driver`, `persistence.*`). PostgreSQL/Redis via `externalPostgresql.*` / `externalRedis.*` — les sous-charts Bitnami ne sont volontairement **pas** embarqués
