...
### Added
- **Tests d'intégration frontend (Playwright)** — dernier item du checklist beta. `@playwright/test` (Chromium), config à deux `webServer` (backend Go en mode démo sur le port 8090, frontend Vite sur 3001 — ports dédiés, distincts des ports de dev habituels pour ne jamais réutiliser par erreur une instance déjà lancée avec des données différentes). `e2e/run-backend.mjs` : le backend démarre sur une base SQLite fraîche à chaque run (répertoire temporaire unique via `mkdtempSync`, pas de nettoyage nécessaire) — un `webServer` shell classique ne peut pas garantir ça de façon portable (pas de `rm` sur `cmd.exe`). 21 tests : authentification (redirection non connecté, identifiants invalides, connexion, déconnexion), fumée de navigation sur les 14 routes protégées, cycle complet créer/mettre en pause/supprimer d'une exception rule via l'UI réelle (formulaire, table, `window.confirm`).
  - **A trouvé un vrai bug produit** : l'intercepteur de réponse Axios redirigeait vers `/login` (rechargement complet de la page) sur **tout** 401 — y compris celui renvoyé par `/auth/login` lui-même en cas de mauvais mot de passe, ce qui effaçait le message d'erreur avant même que `Login.tsx` ait pu l'afficher. Corrigé : le 401 de `/auth/login` est maintenant exclu de la redirection automatique
  - `tsconfig.json` dédié dans `e2e/` (le `tsconfig.json` racine ne couvre que `src/`) pour que les fichiers de test soient bien vérifiés par `tsc`
  - `npm run test` (déjà appelé par `make test-frontend`, jusqu'ici sans script `test` défini) lance la suite ; `npm run test:ui` pour le mode interactif
- **Couverture de tests du reste du `store`** — dernier morceau du checklist beta côté tests (`clusters`, `helm releases/repos`, `registries`, `namespaces`, `métriques`, `secrets`, `integrations`, `exception_rules`). ~40 tests dans 9 nouveaux fichiers, tous sur une base SQLite migrée via `models.AllModels()` (`store_test_helpers_test.go`, nouveau helper partagé). A trouvé et fait corriger 3 bugs réels au passage — voir Fixed.
- **Page History (`/history`)** — l'API `GET /api/v1/action-logs` existait depuis les fondations de la piste d'audit, sans page dédiée (`disabled`, tag `V2` dans la sidebar). Table paginée (50/page) filtrable par action/type d'entité/statut, ligne cliquable → slide-over avec le détail complet (`details` JSONB formaté, IP, user-agent). Aucun changement backend — la page consomme l'endpoint existant tel quel. `tsc`/build propres, validé en direct contre une instance démo fraîche (filtre par statut, pagination, forme de la réponse conforme aux types)
- **Scoring engine — les 3 briques qui manquaient face à `docs/scoring.md`** (trouvées en écrivant les tests de la session précédente, implémentées dans celle-ci) :
  - **Facteur CVSS (poids 20 %)** : câblé en dur à 0 jusqu'ici. Lit désormais `UpdateFinding.CVEs` (JSONB, tableau de `{id, cvss}`), prend le CVSS **maximum** parmi les CVE associées, applique `(cvss_max/10)×20` — formule exacte de `docs/scoring.md` §4. Rien n'alimente encore cette colonne (le scanner Trivy/Grype reste V2, cf. roadmap `CLAUDE.md`) : cette brique rend la formule correcte pour quand ça arrivera, elle ne fait pas apparaître de score par elle-même aujourd'hui
  - **Fenêtre de maintenance réellement évaluée** : `isWindowActive` retournait toujours `false` (« Placeholder — safe default »). Nouvelle dépendance `github.com/robfig/cron/v3` : la planification cron de chaque `MaintenanceWindow` est résolue pour de vrai, avec prise en compte du fuseau horaire (`Timezone`, repli UTC si invalide) et de la durée (`Duration`, minutes) — une fenêtre est active si un déclenchement cron est survenu il y a moins de `Duration` minutes
  - **`ExceptionRule` (suppress / reduce_severity / accept_risk)** — jusqu'ici un modèle sans aucune logique métier (zéro lecture/écriture ailleurs dans le code) et **sans même de colonne `rule_type`**, donc irréalisable tel quel malgré sa description dans `docs/scoring.md` §9. Le modèle gagne `Name`, `RuleType` et `NamespaceName` (migration `011_exception_rule_type.sql`, colonnes ajoutées par `AutoMigrate` sur tous les backends). Le moteur de scoring sélectionne, après le calcul du score de base, la règle active/non expirée dont le scope correspond le plus précisément (`workload_id` > `image_pattern` > `cluster_id`+`namespace_name` > `cluster_id` > global) et applique son effet : `suppress` (score 0, sévérité info), `reduce_severity` (sévérité baissée d'un cran), `accept_risk` (statut forcé à `ignored`, ré-affirmé à chaque passe de scoring tant que la règle reste active). Nouveaux `store.CreateExceptionRule`/`store.ListActiveExceptionRules` — **pas d'endpoint REST ni d'UI de gestion dans ce lot**, les règles s'insèrent directement en base pour l'instant
  - 12 nouveaux tests (`internal/scoring/engine_test.go`), dont une reproduction exacte de l'Example 1 chiffré de `docs/scoring.md` §8 (score 82.6, critical) de bout en bout — la formule documentée et le code sont désormais vérifiés identiques, plus seulement en théorie
- **Exception rules : endpoint REST + page de gestion** — suite directe du lot précédent, qui n'avait volontairement pas d'API/UI (« les règles s'insèrent directement en base »). `GET/POST/PUT/DELETE /api/v1/exception-rules` (lecture ouverte à tous les rôles authentifiés — savoir ce qui est exclu et pourquoi n'a rien de sensible ; écriture réservée `operator`/`admin`, sans distinction supplémentaire sur le delete contrairement aux dépôts Helm). Nouveaux `store.ListExceptionRules`/`GetExceptionRule`/`UpdateExceptionRule`/`DeleteExceptionRule` (préchargent `Cluster`/`Workload` pour affichage). Pas de `RecordAction` — même traitement que les autres CRUD de configuration (dépôts Helm, registries, intégrations) : réservé aux actions qui touchent réellement un cluster
  - **UI** : nouvelle page **Exceptions** (`/exception-rules`, entrée sidebar) — table des règles (effet, scope résolu en texte lisible, raison, expiration, statut actif/en pause/expiré), formulaire de création/édition (5 types de scope : global, cluster, cluster+namespace, workload — avec sélecteur de workload alimenté par cluster choisi, pattern d'image), bouton pause/reprise réutilisant le même endpoint `PUT` que l'édition complète
  - Validé en direct (mode démo, cycle complet) : règle globale et règle scopée à un cluster créées, jointure `cluster` visible dans la réponse liste, `rule_type` invalide rejeté en 400, pause via `PUT` confirmée, `viewer` lit (200) mais ne peut pas créer (403), suppression confirmée (204 puis recompte) — jusqu'ici, changer le statut d'un finding (`planned`/`approved`/`ignored`...) ne posait qu'une étiquette (`PATCH /findings/:id/status`, pur bookkeeping). Nouveau `POST /api/v1/findings/:id/remediate` applique le correctif **réel** : bump du tag d'image sur le Deployment/StatefulSet/DaemonSet concerné (findings image) ou upgrade Helm vers `latest_version` (findings Helm), en réutilisant les values actuelles. Réservé aux findings en statut actionnable (`open`/`planned`/`approved` — 400 sinon), écriture `operator`/`admin`
  - **Nouvelle capacité `k8sops.SetContainerImage`** (`kubectl set image`-équivalent) — patch **strategic merge**, pas merge-patch classique : `spec.template.spec.containers` est une liste, un JSON merge patch la remplacerait entièrement et effacerait les autres conteneurs d'un pod multi-conteneurs. Testé explicitement contre ce cas (sidecar intact après le patch). Aucun changement RBAC (même droits `update`/`patch` que scale/restart)
  - **Résolution rapide** : un correctif réel ne repasse le finding en `resolved` qu'au prochain passage du watcher concerné (jusqu'à `WORKER_INTERVAL_SECONDS`, 300s par défaut) — après le fix, une re-vérification ciblée est relancée en tâche de fond (3 tentatives, 3s/6s/12s) via `CheckImage`/`CheckRelease` (méthodes déjà exportées des watchers, jusqu'ici seulement appelées depuis leur propre boucle), pour une résolution en quelques secondes au lieu de plusieurs minutes
  - **UI** : `FindingDetail` remplace l'ancien lien "commande Helm à copier-coller" (rien d'actionnable) par un vrai bouton **"Fix now → v{latest_version}"**, avec confirmation puis état "Applied — resolving…"
- **Deploy d'un manifest (server-side apply)** — dernière brique du Pilotage MVP : `POST /api/v1/clusters/:id/manifests/apply` (`{"manifest", "namespace"?, "dry_run"?, "force"?}`) applique un manifest YAML brut (un ou plusieurs documents) directement sur un cluster, via le mécanisme natif `kubectl apply --server-side` (`PATCH` en `types.ApplyPatchType`, field manager `kubepilot`). Chaque document est traité indépendamment (comme `kubectl apply -f multi.yaml`) : un document invalide n'empêche pas les autres d'être appliqués, le détail par ressource (kind/nom/namespace/opération `created`|`updated`|`error`) est renvoyé et journalisé dans `action_logs`. `dry_run` (aucun changement, pas de journalisation) permet une prévisualisation avant application réelle ; `force` résout un conflit de field-manager avec un autre contrôleur (défaut `false`, même sémantique par défaut que `kubectl`, qui échoue plutôt que d'écraser silencieusement). Écriture réservée `operator`/`admin`
  - **Aucune nouvelle dépendance ni migration** : `k8s.io/client-go/dynamic` fait déjà partie de `k8s.io/client-go` (nouveau package `internal/k8sops/manifest.go`, dynamic client + RESTMapper construits à la volée depuis `CollectorManager.GetRESTConfig`, déjà exposé pour Helm). Le résultat de l'apply part dans `action_logs.details` (JSONB), pas de nouveau modèle
  - **Aucun changement RBAC nécessaire** : le `ClusterRoleBinding` vers `cluster-admin` déjà accordé pour Helm upgrade/rollback (`kubepilot-gitops`, flag `rbac.helmAdmin`) couvre déjà ce cas — un manifest arbitraire n'est pas plus dangereux qu'un chart Helm arbitraire (même surface : n'importe quelle ressource, CRD comprises)
  - **UI** : nouvelle page **Deploy** (`/deploy`, entrée sidebar réservée à `operator`/`admin`) — sélecteur de cluster, namespace par défaut, textarea YAML, boutons Preview (dry-run) et Apply (avec confirmation), panneau de résultats par ressource. Même correctif `TriggerSync`/re-invalidation différée que scale/restart et Helm (les workloads/releases touchés par un manifest ne sont pas surveillés en temps réel)
- **Helm : édition des values, upgrade, rollback réels** — dernière brique du Pilotage MVP, la plus proche du cœur de la vision produit. Nouvelle dépendance `helm.sh/helm/v3` (v3.14.4, pin `k8s.io/*` en v0.29.0 — compatible sans rétrograder nos v0.29.3 existants). `POST /api/v1/helm/:id/upgrade` (`{"chart_version"?, "values"?}` — les deux optionnels : version vide = garde la version installée, values absentes = garde les values actuelles au lieu de les effacer ; un seul endpoint sert donc à la fois « éditer les values » et « changer de version », Helm n'a pas de distinction) et `POST /api/v1/helm/:id/rollback` (`{"revision"}`). Écriture réservée `operator`/`admin`, journalisée via `RecordAction`
  ...
  - **UI** : page Helm réécrite avec des vraies actions upgrade (values en JSON + version de chart, préremplies avec la version actuelle ; bouton « Use latest » quand un finding Helm existe déjà pour la release) et rollback (numéro de révision), masquées pour le rôle `viewer`. Au passage, correction d'un bug préexistant — le type frontend `HelmRelease` référençait des champs que l'API n'a jamais renvoyés (`release_name`, `namespace_id`, `available_version`, filtre `has_updates`) au lieu des vrais champs (`name`, `namespace_name`, objet `cluster` imbriqué) : plusieurs colonnes/lignes du détail s'affichaient vides depuis un moment. Le filtre « Updates available only », non-fonctionnel car appuyé sur ce champ fantôme, est retiré (remplacé par le rapprochement réel avec les findings Helm dans le détail d'une release)
...
### Security
- **Passe de sécurité complète** (revue en 3 phases : identification par agents parallèles, filtration indépendante par vulnérabilité, seuil de confiance ≥8) — 7 failles confirmées, corrigées ensemble :
  - **SSRF non authentifié dans les watchers image et Helm** *(sévérité la plus haute — aucune session KubePilot requise)* : `ImageWatcher.fetchTags` construit une requête HTTP sortante à partir du registre/repository lu dans la spec d'un workload, et `HelmWatcher.discoverViaArtifactHub` fait de même à partir d'URLs renvoyées par la recherche Artifact Hub — les deux tournent sur une boucle temporisée, sans utilisateur authentifié dans la chaîne. Nouveau package `internal/netguard` : les deux watchers (et `probeHelmRepository`, exposition moindre car déclenché par un `operator`) dialoguent désormais via un `net.Dialer.Control` qui refuse loopback/link-local/multicast — **y compris `169.254.169.254`, l'endpoint de métadonnées cloud AWS/GCP/Azure/OCI** — en validant l'IP réellement composée après résolution DNS (résiste au DNS rebinding). Les plages RFC1918 (`10/8`, `172.16/12`, `192.168/16`) restent volontairement autorisées : les registries/dépôts de charts auto-hébergés sur un réseau privé sont l'usage principal de l'outil (voir infra Harbor à `10.0.60.152`), pas un cas à bloquer
  - **`JWT_SECRET` par défaut codé en dur** (`"change-me-in-production"`) : n'importe qui pouvait forger un token admin valide contre toute instance n'ayant pas surchargé la variable d'environnement. En son absence, un secret aléatoire de 32 octets est désormais généré à chaque démarrage (`config.generateRandomSecret`, `crypto/rand`), avec un `logger.Warn` explicite au boot — compromis assumé : les sessions ne survivent plus à un redémarrage tant que `JWT_SECRET` n'est pas fixé explicitement (attendu en production et pour un usage local persistant)
  - **`Values` des releases Helm exposées à `viewer`** : `GET /helm` et `GET /helm/:id` renvoyaient les values en clair (mots de passe de base de données, clés d'API… un usage très courant des values Helm) à n'importe quel utilisateur authentifié, y compris `viewer`, alors que l'upgrade qui en aurait l'usage légitime est déjà réservé à `operator`/`admin`. Masquées désormais pour les rôles sous `operator`
  - **URL de webhook d'intégration exposée à `viewer`** : `GET /integrations` renvoyait `url` en clair — pour Slack/PagerDuty/Teams, cette URL **est** le jeton d'authentification, pas un simple identifiant. Retirée de la réponse API pour tous les rôles (remplacée par `has_url: bool`), même traitement que les identifiants de dépôt Helm (jamais renvoyés, seulement `has_credentials`)
  - **Aucune révocation de session** : `JWTAuth` ne validait que la signature JWT, sans jamais revérifier l'état en base — désactiver un compte ou changer son rôle via `PATCH /auth/users/:id` (fonctionnalité déjà en prod) n'avait donc aucun effet avant l'expiration naturelle du token (jusqu'à 24 h). Le middleware relit désormais `role`/`is_active` en base à **chaque requête** (`store.ActiveUserRole`, nouvelle interface `middleware.ActiveUserRole`) au lieu de faire confiance aux claims signées à la connexion — un compte désactivé ou rétrogradé perd l'accès immédiatement
- **`Cluster.KubeconfigRef` n'est plus renvoyé par l'API** — ce champ contient le kubeconfig complet (clé privée de certificat client incluse, pas qu'une « référence » malgré le nom), mais n'avait pas le tag `json:"-"` contrairement à `SSHPassword`, déjà correctement redacté. `GET /api/v1/clusters` et `GET /api/v1/clusters/:id` le renvoyaient donc en clair à tout utilisateur authentifié. Découvert en testant une autre fonctionnalité contre un cluster enregistré avec un kubeconfig Teleport multi-clusters personnel

### Fixed
- **`UpsertNode` et `UpsertSecret` pouvaient renvoyer un ID fantôme à l'appelant** — même défaut que l'incident `UpsertWorkload` du 2026-08-01 (voir plus bas), non détecté jusqu'ici faute de test : les deux ne faisaient confiance qu'à `ON CONFLICT DO UPDATE`, qui ne renvoie pas l'ID de la ligne survivante à Go — la struct passée en argument gardait donc l'UUID fraîchement généré au lieu du vrai ID en base. Sans conséquence visible aujourd'hui (rien n'utilise l'ID juste après l'appel dans `internal/collector`), mais un piège prêt à se redéclencher pour tout futur code qui s'y fierait (ex. une FK future vers `Node`/`Secret`). Corrigé avec le même correctif que `UpsertWorkload` : relecture de l'ID existant par clé naturelle avant l'`ON CONFLICT`. Trouvé par `TestUpsertNode_StablePrimaryKeyAcrossPasses`/`TestUpsertSecret_OnConflict`
- **`ExceptionRule` créée avec `is_active:false` redevenait active** : `IsActive bool` porte un tag `gorm:"default:true"` — GORM substitue silencieusement la valeur par défaut à l'insertion dès que le champ Go vaut sa valeur zéro (`false` pour un bool), sans distinction possible entre « pas renseigné » et « explicitement à false », et ce quel que soit `Select`/`Omit` (vérifié directement dans les sources de `gorm.io/gorm@v1.25.10/callbacks/create.go`). `store.CreateExceptionRule` prend désormais un paramètre `isActive bool` explicite et séparé du champ ambigu, avec une correction en un `UPDATE` ciblé après la création si `false` était demandé. Trouvé par `TestListActiveExceptionRules_FiltersExpiredAndInactive`
