...
### Added
- **Fondations de la piste d'audit (`action_logs`)** : nouvel endpoint `GET /api/v1/action-logs` (paginé, filtrable par `user_id`/`action`/`entity_type`/`entity_id`/`status`) et helper `handlers.RecordAction` que les futurs endpoints d'écriture (deploy de manifest, édition/upgrade/rollback Helm, scale/rolling-restart — **aucun n'existe encore**) appelleront pour journaliser chaque tentative. Champ `status` (`success`/`failure`) ajouté à `ActionLog`, qui n'avait jusqu'ici aucun moyen de distinguer une action réussie d'un échec (migration `009_action_log_status.sql`). L'application du rôle reste `middleware.RequireRole`, inchangée ; aucune action d'écriture concrète n'est encore branchée dessus — pure pose de fondations avant deploy/scale/helm-upgrade
